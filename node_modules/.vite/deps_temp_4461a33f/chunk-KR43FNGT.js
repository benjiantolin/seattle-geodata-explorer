...
vec4 blendStipple(vec4 color, float stippleAlpha) { return color; }`);
  ...
  c3.draped || (d(S, c3), S.uniforms.add(new r2("worldToScreenPerDistanceRatio", ({ camera: e8 }) => 1 / e8.perScreenPixelRatio)).code.add(t`float computeWorldToScreenRatio(vec3 segmentCenter) {
...
return worldToScreenPerDistanceRatio / segmentDistanceToCamera;
  ...
  `), h2.uniforms.add(new e("stipplePatternTexture", (e8) => e8.stippleTexture), new r("stipplePatternPixelSizeInv", (e8) => 1 / f3(e8))), c3.stippleOffColorEnabled && h2.uniforms.add(new e3("stippleOffColor", (e8) => s3(e8.stippleOffColor))), p2.include(d3), h2.code.add(t`float getStippleSDF(out bool isClamped) {
...
vTransitionType = timeStamps.w;`);
  ...
  3 === v && T.constants.add("decayRate", "float", 2.3), T.code.add(t`
    float getTrailOpacity(float x) {
      ${s4(v)}
    }`), T.uniforms.add(new r("timeElapsed", (e8) => e8.timeElapsed), new r("trailLength", (e8) => e8.trailLength), new r("speed", (e8) => e8.animationSpeed), new e3("timingOptions", (t6) => s(n4, t6.startTime, t6.endTime, t6.fadeInTime, t6.fadeOutTime))), T.code.add(t`float fadeIn(float x) {
...
vReverseSegmentSDF = noPerspectiveWrite((isJoin && !isStartVertex) ? LARGE_HALF_FLOAT : (dot(pos.xy - segmentEnd, -segmentDir)), pos.w);`), O && (V ? R.uniforms.add(new r2("worldToScreenRatio", (e8) => 1 / e8.screenToPCSRatio)) : R.main.add(t`vec3 segmentCenter = mix((nextPosition + position) * 0.5, (position + prevPosition) * 0.5, isEndVertex);
...
float segmentLengthPseudoScreen = segmentLengthRender * discreteWorldToScreenRatio;`), R.uniforms.add(new r("stipplePatternPixelSize", (e8) => f3(e8))), R.main.add(t`float patternLength = patternLineSize * stipplePatternPixelSize;
...
color.a = noPerspectiveRead(color.a);`), _ && F.uniforms.add(new e3("innerColor", (e8) => e8.innerColor ?? e8.color), new r("innerWidth", (e8, i3) => e8.innerWidth * i3.camera.pixelRatio)).main.add(t`float distToInner = abs(lineDistance) - innerWidth;
float innerAA = clamp(0.5 - distToInner, 0.0, 1.0);
float innerAlpha = innerColor.a + color.a * (1.0 - innerColor.a);
color = mix(color, vec4(innerColor.rgb, innerAlpha), innerAA);`), F.main.add(t`vec4 finalColor = blendStipple(color, stippleAlpha);`), M && (F.uniforms.add(new r("falloff", (e8) => e8.falloff)), F.main.add(t`finalColor.a *= pow(max(0.0, 1.0 - abs(lineDistanceNorm)), falloff);`)), O || F.main.add(t`float featherStartDistance = max(lineWidth - 2.0, 0.0);
...
//# sourceMappingURL=chunk-KR43FNGT.js.map
