...
function O(e5) {
  ...
  const t2 = i.fragment;
  if (e5.lineVerticalPlaneEnabled || e5.heightManifoldEnabled) if (t2.uniforms.add(new r2("maxPixelDistance", (i2, t3) => e5.heightManifoldEnabled ? 2 * t3.camera.computeScreenPixelSizeAt(i2.heightManifoldTarget) : 2 * t3.camera.computeScreenPixelSizeAt(i2.lineVerticalPlaneSegment.origin))), t2.code.add(t`float planeDistancePixels(vec4 plane, vec3 pos) {
...
return abs(dist);
    ...
    t2.uniforms.add(new e3("heightManifoldOrigin", (t3, n4) => (e6(G, t3, n4), i2(_2, n4), c(_2, _2, G), A(W, _2), W[3] = r(_2), W)), new e2("globalOrigin", (e7) => i2(G, e7)), new r2("cosSphericalAngleThreshold", (e7, i3) => 1 - Math.max(2, p(i3.camera.eye, e7.heightManifoldTarget) * i3.camera.perRenderPixelRatio) / r(e7.heightManifoldTarget))), t2.code.add(t`float globeDistancePixels(float posInGlobalOriginLength) {
...
return planeDistancePixels(heightPlane, pos);
  ...
  if (e5.pointDistanceEnabled && (t2.uniforms.add(new r2("maxPixelDistance", (e6, i2) => 2 * i2.camera.computeScreenPixelSizeAt(e6.pointDistanceTarget))), t2.code.add(t`float sphereDistancePixels(vec4 sphere, vec3 pos) {
float dist = distance(sphere.xyz, pos) - sphere.w;
float width = fwidth(dist);
dist /= min(width, maxPixelDistance);
return abs(dist);
}`)), e5.intersectsLineEnabled && t2.uniforms.add(new r3("perScreenPixelRatio", (e6) => e6.camera.perScreenPixelRatio)).code.add(t`float lineDistancePixels(vec3 start, vec3 dir, float radius, vec3 pos) {
...
}`)), e5.intersectsLineEnabled && (t2.uniforms.add(new e4("angleCutoff", (e6) => E2(e6)), new e("intersectsLineStart", (e6, i2) => E(G, e6.lineStartWorld, i2.camera.viewMatrix)), new e("intersectsLineEnd", (e6, i2) => E(G, e6.lineEndWorld, i2.camera.viewMatrix)), new e("intersectsLineDirection", (e6, i2) => (s2(W, e6.intersectsLineSegment.vector), W[3] = 0, A(G, z(W, W, i2.camera.viewMatrix)))), new r2("intersectsLineRadius", (e6) => e6.intersectsLineRadius)), t2.main.add(t`if (pointIsWithinLine(pos, intersectsLineStart, intersectsLineEnd)) {
...
//# sourceMappingURL=chunk-6YZ3FJ4T.js.map
