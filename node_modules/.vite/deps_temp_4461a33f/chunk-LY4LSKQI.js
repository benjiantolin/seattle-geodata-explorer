...
import {
  ...
  o3 as o6,
  t as t4
} from "./chunk-Z3UNDVAZ.js";
import {
  o as o3
} from "./chunk-EPNL5PLK.js";
import {
  n as n3
} from "./chunk-O53PZTKF.js";
import {
  r as r4
...
import {
  p
} from "./chunk-YYVTWQYI.js";
import {
  d,
  f
} from "./chunk-QNEHUG6R.js";
import {
  e as e5
...
function u(t6) {
  t6.fragment.uniforms.add(new e2("texWaveNormal", (e12) => e12.waveNormal), new e2("texWavePerturbation", (e12) => e12.wavePerturbation), new e5("waveParams", (e12) => s(c, e12.waveStrength, e12.waveTextureRepeat, e12.flowStrength, e12.flowOffset)), new e7("waveDirection", (t7) => o(n5, t7.waveDirection[0] * t7.waveVelocity, t7.waveDirection[1] * t7.waveVelocity))), t6.fragment.include(t5), t6.fragment.code.add(t2`const vec2  FLOW_JUMP = vec2(6.0/25.0, 5.0/24.0);
...
function d4(d5, n6) {
  ...
  c2.include(a), c2.uniforms.add(new e3("nearFar", (e12) => e12.camera.nearFar), new e6("depthMap", (e12) => {
    var _a;
    return (_a = e12.depth) == null ? void 0 : _a.attachment;
  }), new t3("proj", (e12) => e12.camera.projectionMatrix), new r3("invResolutionHeight", (e12) => 1 / e12.camera.height), new t3("reprojectionMatrix", (e12) => e12.ssr.reprojectionMatrix)).code.add(t2`
...
function e10(e12) {
  e12.fragment.uniforms.add(new r3("cloudAbsorption", (r6) => r6.clouds.absorption), new r3("cloudCoverage", (r6) => r6.clouds.coverage)).code.add(t2`vec4 lookupCloudsFromTextureArray(sampler2DArray cubeMap, vec3 rayDir) {
int faceIndex;
vec2 uv;
if(rayDir.z <= 0.0) {
...
}`), e12.uniforms.add(new r3("radiusCurvatureCorrection", ({ clouds: o7 }) => o7.parallax.radiusCurvatureCorrection)).code.add(t2`vec3 correctForPlanetCurvature(vec3 dir) {
...
}`), e12.uniforms.add(new e4("anchorPoint", (o7) => o7.clouds.parallax.anchorPoint), new e4("anchorPointNew", (o7) => o7.clouds.parallaxNew.anchorPoint), new t3("rotationClouds", (o7) => o7.clouds.parallax.transform), new t3("rotationCloudsNew", (o7) => o7.clouds.parallaxNew.transform), new r3("cloudsOpacity", (o7) => o7.clouds.opacity), new r3("fadeFactor", (o7) => o7.clouds.fadeFactor), new r4("crossFade", (o7) => 3 === o7.clouds.fadeState)).code.add(t2`vec4 renderClouds(vec3 worldRay, vec3 cameraPosition) {
...
function m4(m5, v) {
  ...
  u2.include(a3, v), u2.include(e), u2.include(n4), v.cloudReflections && m5.include(m3), m5.include(d4, v), u2.include(o6, v), u2.constants.add("fresnelSky", "vec3", [0.02, 1, 15]), u2.constants.add("fresnelMaterial", "vec2", [0.02, 0.1]), u2.constants.add("roughness", "float", 0.015), u2.constants.add("foamIntensityExternal", "float", 1.7), u2.constants.add("ssrIntensity", "float", 0.65), u2.constants.add("ssrHeightFadeStart", "float", e8), u2.constants.add("ssrHeightFadeEnd", "float", o5), u2.constants.add("waterDiffusion", "float", 0.92), u2.constants.add("waterSeaColorMod", "float", 0.8), u2.constants.add("correctionViewingPowerFactor", "float", 0.4), u2.constants.add("skyZenitColor", "vec3", [0.52, 0.68, 0.9]), u2.constants.add("skyColor", "vec3", [0.67, 0.79, 0.9]), u2.constants.add("cloudFresnelModifier", "vec2", [1.2, 0.01]), u2.code.add(t2`PBRShadingWater shadingInfo;
vec3 getSkyGradientColor(in float cosTheta, in vec3 horizon, in vec3 zenit) {
float exponent = pow((1.0 - cosTheta), fresnelSky[2]);
return mix(zenit, horizon, exponent);
}`), u2.uniforms.add(new r3("lightingSpecularStrength", (e12) => e12.lighting.mainLight.specularStrength), new r3("lightingEnvironmentStrength", (e12) => e12.lighting.mainLight.environmentStrength)), u2.code.add(t2`vec3 getSeaColor(in vec3 n, in vec3 v, in vec3 l, vec3 color, in vec3 lightIntensity, in vec3 localUp, in float shadow, float foamIntensity, vec3 viewPosition, vec3 position) {
...
vec3 reflectedWorld = normalize(reflect(-v, normalCorrectedClouds));`), v.cloudReflections && u2.uniforms.add(new r3("cloudsOpacity", (e12) => e12.clouds.opacity)).code.add(t2`vec4 cloudsColor = renderClouds(reflectedWorld, position);
cloudsColor.a = 1.0 - cloudsColor.a;
cloudsColor = pow(cloudsColor, vec4(GAMMA));
cloudsColor *= clamp(fresnelModifier.y * cloudFresnelModifier[0] - cloudFresnelModifier[1], 0.0, 1.0) * cloudsOpacity;`), v.screenSpaceReflections ? u2.uniforms.add(new t3("view", (e12) => e12.camera.viewMatrix), new e6("lastFrameColorTexture", (e12) => {
    var _a;
    return (_a = e12.ssr.lastFrameColor) == null ? void 0 : _a.getTexture();
  }), new r3("fadeFactorSSR", (e12) => e12.ssr.fadeFactor)).code.add(t2`vec3 viewDir = normalize(viewPosition);
...
function L(L2) {
  ...
  const _ = new e5("waterColor", (e12) => e12.color);
  ...
  switch (C2.include(f2, L2), C2.include(i2, L2), O) {
    case 0:
    case 1:
      C2.include(m, { pbrMode: 0, lightingSphericalHarmonicsOrder: 2 }), C2.include(u), C2.include(m4, L2), x.include(p, L2), C2.include(n3, L2), x.include(e9), d(x, L2), t4(x), a2(x), x.uniforms.add(_, new r2("timeElapsed", ({ timeElapsed: e12 }) => e12), P.uniforms.get("view"), P.uniforms.get("localOrigin")).main.add(t2`
        discardBySlice(vpos);
    ...
    case 3:
      C2.include(r5, L2), C2.include(u, L2), x.include(p, L2), S.add("vuv", "vec2"), P.main.add(t2`
        ...
        gl_Position = transformPosition(proj, view, vpos);`), x.uniforms.add(new r2("timeElapsed", ({ timeElapsed: e12 }) => e12)).main.add(t2`discardBySlice(vpos);
vec4 tangentNormalFoam = getSurfaceNormalAndFoam(vuv, timeElapsed);
...
//# sourceMappingURL=chunk-LY4LSKQI.js.map
