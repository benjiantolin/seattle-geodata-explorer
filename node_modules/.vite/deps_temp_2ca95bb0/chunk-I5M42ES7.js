...
function r(r2) {
  ...
  if ("string" != typeof r2.value) throw new n2(null, "InvalidIdentifier", null);
  return r2.value.toLowerCase();
}
var t2 = Object.freeze({ aborted: false });
...
async function n3(n4, e3, p2) {
  ...
  return (await H(m2.path, u)).data.results.map((t3) => s3.fromJSON(t3));
}
...
var d = class {
  ...
  async translate(t3) {
    ...
    try {
      t3.requestSource ?? (t3.requestSource = "arcade");
      return { success: true, results: (await n3(s5, t3, { authMode: "no-prompt" })).map((t4) => t4.toJSON()) };
    } catch (r2) {
...
var g = class {
  constructor(t3, e3, s5) {
    this._parameters = t3, this._maxTotalContentSize = e3, this._maxContentsLength = s5, this._requests = [], this._normalizedContents = /* @__PURE__ */ new Map(), this._contentsTotalSize = 0;
  }
  tryAdd(t3) {
    ...
    for (const { key: n4, text: o2 } of t3) e(this._normalizedContents, o2, () => []).push({ batchIdx: r2, key: n4 });
    return this._requests.push(t3), this._contentsTotalSize += s5, r2;
  }
  async send(t3) {
    const e3 = [], s5 = [];
    let r2 = 0;
    ...
    n4.contents = e3;
    const o2 = await t3.translate(n4);
    ...
    for (const i3 of o2.results) {
      ...
      for (const e4 of s5[t4]) a3[e4.batchIdx].results.push({ ...i3, key: e4.key });
    }
    return a3;
  }
};
function y(t3) {
  const e3 = [...new Set(t3.to)].sort(), s5 = t3.from ?? null, r2 = t3.portalUrl, n4 = t3.translator, o2 = t3.apiKey ?? null;
  return JSON.stringify([e3, s5, r2, n4, o2]);
}
async function w(t3, e3, s5) {
  try {
    return (await t3.yieldFor(s5))[e3];
  } catch {
    return { success: false };
  }
}
var x = class {
  constructor(t3, e3, { maxTotalContentSize: s5 = 5e4, maxContentsLength: r2 = 1e3 } = {}) {
    this._executor = t3, this._service = e3, this._openBatches = /* @__PURE__ */ new Map(), this._maxTotalContentSize = s5, this._maxContentsLength = r2;
  }
  create(t3) {
    return { translate: async (e3) => {
      ...
      if (null == r2 || r2.every((t4) => 0 === t4.text.length)) return { success: false };
      const l = this._openBatches.get(s5);
      if (null != l) {
        const e4 = l.data.tryAdd(r2);
        if (null != e4) return await w(t3, e4, l);
        l.send();
      }
      ...
      if (null != h) {
        const e4 = this._executor.openBatch(u, { open: (t4) => {
          this._openBatches.set(s5, t4);
        }, execute: (t4) => t4.send(this._service), close: (t4) => {
          this._openBatches.get(s5) === t4 && this._openBatches.delete(s5);
        } });
        return await w(t3, h, e4);
      }
...
function S(a3) {
  "async" === a3.mode && (a3.functions[r("TranslateText")] = function(t3, i3) {
    return a3.standardFunctionAsync(t3, i3, async (t4, a4, i4) => {
      var _a, _b, _c, _d, _e2, _f;
      if (oe(i4, 2, 3, t4, a4), !e2(i4[0]) && !o(i4[0]) && !te(i4[0])) throw new n2(t4, "InvalidParameter", a4);
      const c = _e(i4[0]);
      if (!e2(i4[1]) && !o(i4[1]) && !te(i4[1])) throw new n2(t4, "InvalidParameter", a4);
      const u = _e(i4[1]);
      let g2 = null;
      if (i4.length >= 3) {
        if (!e2(i4[2])) throw new n2(t4, "InvalidParameter", a4);
        g2 = i4[2];
      }
      const w2 = c.map((t5, e3) => new s4({ key: String(e3), text: t5 })), x2 = ((_a = t4.services) == null ? void 0 : _a.portal) ?? M.getDefault(), S2 = new i2({ to: u, contents: w2, from: g2, portalUrl: x2.restUrl.replace(/\/sharing\/rest$/, "") }), T2 = /* @__PURE__ */ new Map();
      let v = null;
      if (null != t4.lrucache) {
        const e3 = t4.lrucache;
        v ?? (v = y(S2)), S2.contents = (_b = S2.contents) == null ? void 0 : _b.filter((t5) => {
          const s5 = e3.getCachedTranslation(v, t5.text);
          return null == s5 || (T2.set(t5.key, { ...s5, key: t5.key, text: t5.text }), false);
        });
      }
      if ((_c = S2.contents) == null ? void 0 : _c.length) {
        const r2 = ((_d = t4.services) == null ? void 0 : _d.translation) ?? new d(x2, t4.console), n4 = await r2.translate(S2);
        if (!n4.success) return new p({ __proto__: null, success: false });
        for (const e3 of n4.results) {
          const r3 = (_f = (_e2 = S2.contents) == null ? void 0 : _e2.find((t5) => t5.key === e3.key)) == null ? void 0 : _f.text;
          if (null == r3) throw new n2(null, "NeverReach", null);
          T2.set(e3.key, e3), null != t4.lrucache && (v ?? (v = y(S2)), t4.lrucache.setCachedTranslation(v, r3, { detectedLanguage: e3.detectedLanguage, translation: e3.translation }));
        }
      }
      const C = Array.from(w2, (r2) => {
        const n4 = s3.fromJSON(T2.get(r2.key));
        if (null == n4) throw new n2(null, "NeverReach", null);
        return n4.text = r2.text, p.convertJsonToArcade(n4.toJSON(), t4.timeZone ?? "system", true);
      });
...
//# sourceMappingURL=chunk-I5M42ES7.js.map
