...
function s3(s5) {
  const a8 = s5 == null ? void 0 : s5.match(t);
  if (!a8) return null;
  const [, r4, c4, o2] = a8;
  if (!r4) return null;
...
function A(t5, n3) {
  return t5 ? n3 && "function" == typeof n3 ? Object.keys(t5).map((e5) => encodeURIComponent(e5) + "=" + encodeURIComponent(n3(e5, t5[e5]))).join("&") : Object.keys(t5).map((e5) => {
    ...
    return i4 ? s5 + encodeURIComponent(i4(r4)) : Array.isArray(r4) ? r4.map((t6) => t2(t6) ? s5 + encodeURIComponent(JSON.stringify(t6)) : s5 + encodeURIComponent(t6)).join("&") : t2(r4) ? s5 + encodeURIComponent(JSON.stringify(r4)) : s5 + encodeURIComponent(r4);
  }).filter((t6) => t6).join("&") : "";
}
function v(t5 = false) {
...
function at(t5, n3) {
  ...
  if (!("download" in e5)) return false;
  const r4 = URL.createObjectURL(t5);
  return e5.download = n3, e5.href = r4, e5.style.display = "none", document.body.appendChild(e5), e5.click(), document.body.removeChild(e5), URL.revokeObjectURL(r4), true;
}
function ht(t5) {
...
async function G2() {
  has("host-webworker") && (!M2 && globalThis.invokeStaticMessage ? M2 = await import("./request-WIEWFVPX.js") : R3 = true);
}
async function J2() {
  s4 || await import("./IdentityManager-4GWQ2G7W.js");
}
...
async function te(e5) {
  ...
  if (!r4) return se(e5);
  const { QueueProcessor: t5 } = await import("./QueueProcessor-SMT3SD2N.js"), s5 = e(re, r4.origin, () => {
...
//# sourceMappingURL=chunk-5B2RE5AQ.js.map
