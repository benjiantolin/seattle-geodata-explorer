import {
  I,
  S,
  d,
  g
} from "./chunk-FUDTO63A.js";
import "./chunk-OQ5ADIQX.js";
import {
  o as o2,
  s
} from "./chunk-PPNF744R.js";
import {
  f as f3
} from "./chunk-PBBJ66DO.js";
import {
  p
} from "./chunk-VWLUSMA7.js";
import {
  t
} from "./chunk-HTNUEZP6.js";
import {
  b2 as b,
  y
} from "./chunk-YGNAKL32.js";
import "./chunk-7YH7T6CS.js";
import {
  j as j2
} from "./chunk-7SAUT2YJ.js";
import {
  l as l2
} from "./chunk-5N4FQ62R.js";
import "./chunk-CPMR6QXZ.js";
import "./chunk-KMUJSAU3.js";
import "./chunk-ZK36HSLI.js";
import "./chunk-MSLX33MC.js";
import "./chunk-DFS5OUCZ.js";
import "./chunk-JEQJUYQO.js";
import "./chunk-PMSG6IMG.js";
import "./chunk-ELUHS27E.js";
import "./chunk-4FZU2J2B.js";
import "./chunk-UL3MJS3D.js";
import "./chunk-CLVSZVCH.js";
import "./chunk-5KRYSVOC.js";
import "./chunk-SXZO7ULH.js";
import "./chunk-TTLSEJNG.js";
import "./chunk-ZVTEFYZX.js";
import "./chunk-LPUPM55U.js";
import "./chunk-2WVHQ5YM.js";
import "./chunk-4O3MA5ZG.js";
import "./chunk-R7ZMFDHQ.js";
import "./chunk-FHHHRIDK.js";
import "./chunk-6LIHUNPJ.js";
import "./chunk-3LWABE6N.js";
import "./chunk-HAQ24E5R.js";
import "./chunk-662ESHWD.js";
import "./chunk-QCVOQIXS.js";
import "./chunk-QMIKXPG2.js";
import "./chunk-3ELPEKJ7.js";
import "./chunk-FKOO5BGL.js";
import "./chunk-JSZFV2PL.js";
import "./chunk-7M4F47TP.js";
import "./chunk-FFTN6XP6.js";
import "./chunk-OPO5EWVP.js";
import "./chunk-XFJNZ4XB.js";
import "./chunk-LCQJHZCS.js";
import "./chunk-EK7HOTRR.js";
import {
  l as l3
} from "./chunk-B6DJZBIB.js";
import "./chunk-Z3KC2EN2.js";
import "./chunk-S4N42LQB.js";
import "./chunk-D4737OGM.js";
import "./chunk-42HHREDE.js";
import "./chunk-T3SF2DG4.js";
import "./chunk-WRX53DSO.js";
import "./chunk-YLAGKT2D.js";
import "./chunk-MAGXRN4Y.js";
import "./chunk-LDZK4LFE.js";
import "./chunk-23DG3MJ6.js";
import "./chunk-ZNT3H4AP.js";
import "./chunk-WOU6FY66.js";
import "./chunk-2ZZFSGSD.js";
import "./chunk-KU7RB4JF.js";
import "./chunk-RPMTQDCU.js";
import "./chunk-IPLQFFWX.js";
import "./chunk-ZJ4CV3ZP.js";
import "./chunk-XWUQTAZB.js";
import "./chunk-EP5JKPAY.js";
import "./chunk-R62VOCCU.js";
import "./chunk-LDTJ57MC.js";
import "./chunk-HUNCMZQ4.js";
import "./chunk-LQP4WSLI.js";
import "./chunk-K2OZ6WVC.js";
import "./chunk-EF2ZYDF2.js";
import "./chunk-K6QPWYEK.js";
import "./chunk-NV4FBUWV.js";
import "./chunk-FNWWWR7D.js";
import "./chunk-G6ZQDBBN.js";
import "./chunk-KV2NAZ22.js";
import "./chunk-UELKSLTU.js";
import "./chunk-FGL4ALAI.js";
import "./chunk-MVHJ7ZMA.js";
import "./chunk-NKOXUP27.js";
import "./chunk-REHMDY45.js";
import "./chunk-CSMCPN64.js";
import "./chunk-XLOINTMG.js";
import "./chunk-JERMP4KV.js";
import "./chunk-75XOUSSS.js";
import "./chunk-HLWZGI5N.js";
import "./chunk-7CBMHDRH.js";
import {
  b as b2
} from "./chunk-IJV5NZUE.js";
import "./chunk-QLP6ZIJ7.js";
import "./chunk-BZO3ITOT.js";
import "./chunk-FC5GJR3N.js";
import {
  O
} from "./chunk-ESZEVXBV.js";
import "./chunk-UCBG5G3C.js";
import {
  u
} from "./chunk-LOECB55L.js";
import "./chunk-26PPD2IM.js";
import "./chunk-X7WBZITA.js";
import "./chunk-PXCQMJ56.js";
import "./chunk-B2PP7IG4.js";
import "./chunk-TJ2V6HKV.js";
import "./chunk-2TWYXDYU.js";
import "./chunk-LTPHDYPK.js";
import "./chunk-3OVERDFX.js";
import "./chunk-TNT36CGI.js";
import "./chunk-AZXJIEZ6.js";
import "./chunk-Q75WYS7K.js";
import "./chunk-SMWUT52Z.js";
import {
  z
} from "./chunk-44372O2O.js";
import {
  o
} from "./chunk-34LIK2UE.js";
import {
  f as f2,
  r
} from "./chunk-HUFIZCRB.js";
import "./chunk-OM56SKRM.js";
import "./chunk-6KIXN4DJ.js";
import "./chunk-7FOYIXEE.js";
import {
  a as a3
} from "./chunk-JSIIAR2E.js";
import {
  U,
  a,
  j,
  l
} from "./chunk-HQNHYZTS.js";
import "./chunk-4IOOERKQ.js";
import {
  n
} from "./chunk-LKEH7BUR.js";
import "./chunk-MRUYFF2S.js";
import "./chunk-RCWIE5HA.js";
import "./chunk-IQYRAWGM.js";
import "./chunk-WAHD3BRT.js";
import "./chunk-CYATH7RF.js";
import {
  m2 as m
} from "./chunk-ZJDN37HI.js";
import {
  a2,
  w
} from "./chunk-KUPQR5T7.js";
import {
  __decorate
} from "./chunk-SS5CLIUY.js";
import "./chunk-YJENKTFP.js";
import "./chunk-DZZB56GD.js";
import "./chunk-PWQME5NH.js";
import "./chunk-X74OCRE6.js";
import "./chunk-LZ2XTYP2.js";
import "./chunk-45CFSCQX.js";
import "./chunk-YOCPKZV2.js";
import {
  qt
} from "./chunk-TERDH76S.js";
import "./chunk-4DGC7CBY.js";
import {
  f2 as f
} from "./chunk-WQ7YCVE7.js";
import {
  c
} from "./chunk-3JNKJ5OW.js";
import "./chunk-DHRRDJZR.js";
import "./chunk-3F74YA3Z.js";

// node_modules/@arcgis/core/graphic/isKMLGraphicOrigin.js
var i = Symbol("isKMLGraphicOrigin");

// node_modules/@arcgis/core/graphic/KMLGraphicOrigin.js
var e;
var s2 = class extends s {
  get [(e = i, o2)]() {
    return this.layer;
  }
  constructor(r2, i2) {
    super(), this[e] = true, this.type = "kml", this.layer = r2, this.sublayer = i2;
  }
  get id() {
    return `${this.layer.id}:__${this.sublayer.id}__`;
  }
};

// node_modules/@arcgis/core/layers/support/KMLSublayer.js
var S2;
var g2 = S2 = class extends n(a3(u)) {
  constructor(...e2) {
    super(...e2), this.description = null, this.fullExtent = null, this.id = null, this.networkLink = null, this.parent = null, this.sublayers = null, this.title = null, this.sourceJSON = null, this.layer = null, this.addHandles([a(() => this.sublayers, "after-add", ({ item: e3 }) => {
      e3.parent = this, e3.layer = this.layer;
    }, U), a(() => this.sublayers, "after-remove", ({ item: e3 }) => {
      e3.layer = e3.parent = null;
    }, U), l(() => this.sublayers, (e3, t2) => {
      if (t2) for (const r2 of t2) r2.layer = r2.parent = null;
      if (e3) for (const r2 of e3) r2.parent = this, r2.layer = this.layer;
    }, U), l(() => this.layer, (e3) => {
      if (this.sublayers) for (const t2 of this.sublayers) t2.layer = e3;
    }, U)]);
  }
  initialize() {
    j(() => this.networkLink).then(() => j(() => true === this.visible)).then(() => this.load());
  }
  load(e2) {
    var _a;
    if (!this.networkLink) return;
    if (this.networkLink.viewFormat) return;
    const r2 = null != e2 ? e2.signal : null, i2 = this._fetchService(((_a = this._get("networkLink")) == null ? void 0 : _a.href) ?? "", r2).then((e3) => {
      var _a2;
      const r3 = I(e3.sublayers);
      this.fullExtent = z.fromJSON(r3), this.sourceJSON = e3;
      const i3 = w(O.ofType(S2), S(S2, e3));
      this.sublayers ? this.sublayers.addMany(i3) : this.sublayers = i3, (_a2 = this.layer) == null ? void 0 : _a2.emit("sublayer-update"), this.layer && this.layer.notifyChange("visibleSublayers");
    });
    return this.addResolvingPromise(i2), Promise.resolve(this);
  }
  get visible() {
    return this._get("visible");
  }
  set visible(e2) {
    this._get("visible") !== e2 && (this._set("visible", e2), this.layer && this.layer.notifyChange("visibleSublayers"));
  }
  readVisible(e2, t2) {
    return !!t2.visibility;
  }
  get origin() {
    return this.layer ? new s2(this.layer, this) : null;
  }
  _fetchService(e2, t2) {
    return g(e2, this.layer.outSpatialReference, this.layer.refreshInterval, t2).then((e3) => d(e3.data));
  }
};
__decorate([m()], g2.prototype, "description", void 0), __decorate([m({ type: z })], g2.prototype, "fullExtent", void 0), __decorate([m()], g2.prototype, "id", void 0), __decorate([m({ readOnly: true, value: null })], g2.prototype, "networkLink", void 0), __decorate([m({ json: { write: { allowNull: true } } })], g2.prototype, "parent", void 0), __decorate([m({ type: O.ofType(g2), json: { write: { allowNull: true } } })], g2.prototype, "sublayers", void 0), __decorate([m({ value: null, json: { read: { source: "name", reader: (e2) => c(e2) } } })], g2.prototype, "title", void 0), __decorate([m({ value: true })], g2.prototype, "visible", null), __decorate([o("visible", ["visibility"])], g2.prototype, "readVisible", null), __decorate([m()], g2.prototype, "sourceJSON", void 0), __decorate([m()], g2.prototype, "layer", void 0), __decorate([m()], g2.prototype, "origin", null), g2 = S2 = __decorate([a2("esri.layers.support.KMLSublayer")], g2);

// node_modules/@arcgis/core/layers/KMLLayer.js
var E = ["kml", "xml"];
var F = class extends p(f3(t(b(j2(l2(b2)))))) {
  constructor(...e2) {
    super(...e2), this._visibleFolders = [], this.allSublayers = new l3({ getCollections: () => [this.sublayers], getChildrenFunction: (e3) => e3.sublayers }), this.outSpatialReference = f2.WGS84, this.path = null, this.legendEnabled = false, this.operationalLayerType = "KML", this.sublayers = null, this.type = "kml", this.url = null;
  }
  initialize() {
    this.addHandles([l(() => this.sublayers, (e2, r2) => {
      r2 && r2.forEach((e3) => {
        e3.parent = null, e3.layer = null;
      }), e2 && e2.forEach((e3) => {
        e3.parent = this, e3.layer = this;
      });
    }, U), this.on("sublayer-update", () => this.notifyChange("fullExtent"))]);
  }
  normalizeCtorArgs(e2, r2) {
    return "string" == typeof e2 ? { url: e2, ...r2 } : e2;
  }
  get sublayerById() {
    const e2 = /* @__PURE__ */ new Map();
    for (const r2 of this.allSublayers) e2.set(r2.id, r2);
    return e2;
  }
  readSublayersFromItemOrWebMap(e2, r2) {
    this._visibleFolders = r2.visibleFolders;
  }
  readSublayers(e2, r2, t2) {
    return S(g2, r2, t2, this._visibleFolders);
  }
  writeSublayers(e2, r2) {
    const t2 = [], o3 = e2.toArray();
    for (; o3.length; ) {
      const e3 = o3[0];
      e3.networkLink || (e3.visible && t2.push(e3.id), e3.sublayers && o3.push(...e3.sublayers.toArray())), o3.shift();
    }
    r2.visibleFolders = t2;
  }
  get title() {
    const e2 = this._get("title");
    return e2 && "defaults" !== this.originOf("title") ? e2 : this.url ? qt(this.url, E) || "KML" : e2;
  }
  set title(e2) {
    this._set("title", e2);
  }
  get visibleSublayers() {
    const e2 = this.sublayers, r2 = [], t2 = (e3) => {
      e3.visible && (r2.push(e3), e3.sublayers && e3.sublayers.forEach(t2));
    };
    return e2 == null ? void 0 : e2.forEach(t2), r2;
  }
  get fullExtent() {
    return this._recomputeFullExtent();
  }
  load(e2) {
    const r2 = null != e2 ? e2.signal : null;
    return this.addResolvingPromise(this.loadFromPortal({ supportedTypes: ["KML"], supportsData: false }, e2).catch(f).then(() => this._fetchService(r2))), Promise.resolve(this);
  }
  destroy() {
    super.destroy(), this.allSublayers.destroy();
  }
  async _fetchService(e2) {
    const r2 = await Promise.resolve().then(() => this.resourceInfo ? { ssl: false, data: this.resourceInfo } : g(this.url ?? "", this.outSpatialReference, this.refreshInterval, e2)), t2 = d(r2.data);
    t2 && this.read(t2, { origin: "service" });
  }
  _recomputeFullExtent() {
    let e2 = null;
    null != this.extent && (e2 = this.extent.clone());
    const r2 = (t2) => {
      if (t2.sublayers) for (const o3 of t2.sublayers.items) r2(o3), o3.visible && o3.fullExtent && (null != e2 ? e2.union(o3.fullExtent) : e2 = o3.fullExtent.clone());
    };
    return r2(this), e2;
  }
};
__decorate([m({ readOnly: true })], F.prototype, "allSublayers", void 0), __decorate([m({ readOnly: true })], F.prototype, "sublayerById", null), __decorate([m({ type: f2 })], F.prototype, "outSpatialReference", void 0), __decorate([m({ type: String, json: { origins: { "web-scene": { read: true, write: true } }, read: false } })], F.prototype, "path", void 0), __decorate([m({ readOnly: true, json: { read: false, write: false } })], F.prototype, "legendEnabled", void 0), __decorate([m({ type: ["show", "hide", "hide-children"] })], F.prototype, "listMode", void 0), __decorate([m({ type: ["KML"] })], F.prototype, "operationalLayerType", void 0), __decorate([m({})], F.prototype, "resourceInfo", void 0), __decorate([m({ type: O.ofType(g2), json: { write: { ignoreOrigin: true } } })], F.prototype, "sublayers", void 0), __decorate([o(["web-map", "portal-item"], "sublayers", ["visibleFolders"])], F.prototype, "readSublayersFromItemOrWebMap", null), __decorate([o("service", "sublayers", ["sublayers"])], F.prototype, "readSublayers", null), __decorate([r("sublayers")], F.prototype, "writeSublayers", null), __decorate([m({ readOnly: true, json: { read: false } })], F.prototype, "type", void 0), __decorate([m({ json: { origins: { "web-map": { read: { source: "title" } } }, write: { ignoreOrigin: true } } })], F.prototype, "title", null), __decorate([m(y)], F.prototype, "url", void 0), __decorate([m({ readOnly: true })], F.prototype, "visibleSublayers", null), __decorate([m({ type: z })], F.prototype, "extent", void 0), __decorate([m()], F.prototype, "fullExtent", null), F = __decorate([a2("esri.layers.KMLLayer")], F);
var O2 = F;
export {
  O2 as default
};
//# sourceMappingURL=KMLLayer-Y6C477FK.js.map
